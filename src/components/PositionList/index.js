...
import { useCsprPrice } from '../../contexts/GlobalData'
...
const DashGrid = styled.div`
  ...
  grid-template-areas: 'number name casperswap return';
  ...
  @media screen and (max-width: 740px) {
    ...
    grid-template-areas: 'name casperswap return';
  }
...
const SORT_FIELD = {
  VALUE: 'VALUE',
  CASPERSWAP_RETURN: 'CASPERSWAP_RETURN',
}
      ...
      <DashGrid style={{ opacity: poolOwnership > 0 ? 1 : 0.6 }} focus={true}>
        ...
        <DataText area="name" justifyContent="flex-start" alignItems="flex-start">
          <AutoColumn gap="8px" justify="flex-start" align="flex-start">
            <DoubleTokenLogo size={16} a0={position.pair.token0.id} a1={position.pair.token1.id} s0={position.pair.token0.symbol} s1={position.pair.token1.symbol} margin={!below740} />
          </AutoColumn>
  ...
  const positionsSorted =
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.CASPERSWAP_RETURN) {
          return p0?.casperswap?.return > p1?.casperswap?.return ? (sortDirection ? -1 : 1) : sortDirection ? 1 : -1
        }
        ...
        {!below500 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            <ClickableText
              area="return"
              onClick={() => {
                setSortedColumn(SORT_FIELD.CASPERSWAP_RETURN)
                setSortDirection(sortedColumn !== SORT_FIELD.CASPERSWAP_RETURN ? true : !sortDirection)
              }}
            >
              {below740 ? 'Fees' : 'Total Fees Earned'}{' '}
              {sortedColumn === SORT_FIELD.CASPERSWAP_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>
